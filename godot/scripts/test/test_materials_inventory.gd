...
func _test_default_state() -> void:
	...
	_assert(GameState.material_count(&"tie") == 0, "missing material -> 0")


func _test_add_consume() -> void:
	_reset()
	GameState.add_material(&"tie", 5)
	_assert(GameState.material_count(&"tie") == 5, "iron=5 after add")
	GameState.add_material(&"tie", 3)
	_assert(GameState.material_count(&"tie") == 8, "iron=8 after add 3")
	var ok := GameState.consume_material(&"tie", 6)
	_assert(ok, "consume 6 returns true")
	_assert(GameState.material_count(&"tie") == 2, "iron=2 after consume 6")
...
func _test_signal_emits() -> void:
	...
	GameState.add_material(&"zhu_sha", 4)
	GameState.consume_material(&"zhu_sha", 1)
	_assert(emitted.size() == 2, "2 signals emitted")
	_assert(emitted[0]["mid"] == &"zhu_sha" and emitted[0]["val"] == 4, "first emit val=4")
	_assert(emitted[1]["mid"] == &"zhu_sha" and emitted[1]["val"] == 3, "second emit val=3")
