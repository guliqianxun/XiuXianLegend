...
- ✅ N0：旧战斗/塔/赛季/卡牌系统归档到 `_deprecated/`（已于 2026-05-05 整理删除）
...
- ✅ UI 易用性 v1：柜台门帘指示器（idle 门外静寂 / pending 门外有客 + 按钮 disabled / just-arrived 朱红 flash + 帘横晃 1s / spawn miss 灰 flash + 1.5s 节流 EventLog "门外无人迹"）+ 星谱 ESC 关闭 + 右上角 ✕
- ✅ 主铺重做 v1：桌面卷宗风（4 张 PanelContainer 卷宗 + 朱红汉字印章【炉/帖/谱/规】+ 实时近况 + hover 抬起 -6px + ±3° 错落倾斜），老铁立绘居中（640, 540），DoorVisual 内嵌门外卷宗
- ✅ 古谱视觉差异化 v1：GuPuData 加 tint_color/accent_color/glyph_char 三字段，7 谱按五行/方位独立配色（青墨青/玄深蓝/朱赤/白银/紫金/血红/残灰），切谱时背景 + 主脉线 + 星点 glow + 右上角朱红印章【青/玄/朱/白/紫/血/残】全部联动；7 谱独立 preset_lines 形状（龙脊/龟蛇环/鸟翅/虎身/帝阵/凶煞/残破）+ 无关星点 modulate.a=0.18 淡化
- ✅ 多分辨率适配 v1：stretch/aspect=expand + window resizable + F11/Alt+Enter 全屏 toggle（WindowManager autoload），shop_screen 监听 viewport size_changed 重布局 4 卷宗 + 老铁，验证 1024×768 / 1280×720 / 1920×1080 / 2560×1440 / 3440×1440 全过 n4 烟测
- ✅ 主线 + 引导 v1：StoryChapters autoload 把 0-15 段身份碎片包装为 5 章（序章·旧债 / 第一卷·盈门 / 第二卷·破谱 / 第三卷·异种 / 终章·真相），HUD 显当前章 + 跨章金色 flash + 大字弹幕；MainQuestPanel 主线追踪面板（章节标 + 碎片进度 + 已解锁碎片正文 + 14 条线索 hint）；OnboardingOverlay 5 步首次引导（高亮 4 卷宗 + 主线按钮 + spotlight 边框）持久化 GameState.onboarding_done
- ✅ 项目清理（2026-05-05）：删除 `_deprecated/` 全部历史归档（76 文件 / -2888 行旧战斗/塔/卡牌/anomaly/idle 残留），36 测试场景仍 0 FAIL
- ⏳ N6-N10：见 spec §13.2
